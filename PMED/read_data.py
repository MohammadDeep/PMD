...
from config import np_dir ,raw_dir
...
def read_segmented_np(label, np_dir = np_dir):
...
def read_synchronised_data(subject_id):
    ...
    filename = Path(raw_dir, f"S_{str(subject_id).zfill(2)}-synchronised-data.csv")
...
if __name__ == "__main__":
    ...
    axes = df.resample(rule= "250ms").mean().plot(subplots=True, figsize= ( 12 ,12))
    plt.legend()
    plt.show()
